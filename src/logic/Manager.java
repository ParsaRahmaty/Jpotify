...
public class Manager {
    ...
    public void makeAndAddSong(String filePath) {
        Song newSong = new Song(filePath, false);
        if (newSong.getMp3File() != null && newSong.getFilePath() != null && !songs.contains(newSong)) {
            songs.add(newSong);
    ...
    public boolean addPlaylistWithoutMessages(Playlist playlist) {
        if (!playlists.contains(playlist)) {
            playlists.add(playlist);
            return true;
        } else {
...
//                    if (!getSongs().contains(new Song(library.getPath() + "\\" + mp3Name)))
    ...
    public void saveObject() throws IOException {
        FileOutputStream fileOutputStream = new FileOutputStream(Manager.getInstance().getMainFrame().getUsername() + ".txt");
    ...
    public void loadObject() {
        try {
            FileInputStream fileInputStream = new FileInputStream(Manager.getInstance().getMainFrame().getUsername() + ".txt");
