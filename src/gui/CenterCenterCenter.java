...
public class CenterCenterCenter extends JPanel {
            ...
            if (addables.get(0) instanceof Song) {
                for (int i = 0; i < addables.size(); i++) {
                    if (!Manager.getInstance().getMainFrame().getSouth().isListed()) {
                        if (((Song) addables.get(i)).getSongName().toLowerCase().contains(searchedText.toLowerCase()) || ((Song) addables.get(i)).getAlbumName().toLowerCase().contains(searchedText.toLowerCase()) || ((Song) addables.get(i)).getArtistName().toLowerCase().contains(searchedText.toLowerCase())) {
                ...
                for (int i = 0; i < addables.size(); i++) {
                    if (!Manager.getInstance().getMainFrame().getSouth().isListed()) {
                        if (((Album) addables.get(i)).getName().toLowerCase().contains(searchedText.toLowerCase()) || ((Album) addables.get(i)).getArtistName().toLowerCase().contains(searchedText.toLowerCase()))
                ...
                for (int i = 0; i < addables.size(); i++) {
                    if (!Manager.getInstance().getMainFrame().getSouth().isListed()) {
                        if (((Playlist) addables.get(i)).getName().toLowerCase().contains(searchedText.toLowerCase()))
    ...
    public void setAddablesToNowPlaying() {
        int size = addables.size();
        for (int i = 0; i < size; i++)
            addables.remove(0);
        for (int i = 0; i < Manager.getInstance().getNowPlaying().size(); i++) {
            addables.add(Manager.getInstance().getNowPlaying().get(i));
        }
        playlist = null;
    }
